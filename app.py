...
sensitive data (stock credentials, other users' orders), onto this server:
  ...
  - Manual payment (customer self-reports a bKash/Nagad transaction id,
    admin eyeballs it and delivers by hand) has been removed. The gateway
    auto-pay path (nstopup create + verify, both server-side) is the only
    way to pay now. The admin "Pending" queue still exists, but it's only
    ever populated when a gateway payment is confirmed and stock happens
    to be empty at that moment - see handle_payment_success().
...
GETTING THE FIREBASE SERVICE ACCOUNT KEY ONTO RENDER:
  Firebase Console -> your project -> gear icon -> Project settings ->
  Service accounts tab -> "Generate new private key". This downloads a
  .json file - a few lines of nested JSON, not something that pastes
  cleanly into a normal single-line env var box. Render has a feature
  built for exactly this ("Secret Files"), so use that instead of a
  regular Environment Variable:
    1. Render dashboard -> your service -> Environment (left sidebar)
    2. Scroll to the "Secret Files" section -> Add Secret File
    3. Filename:  /etc/secrets/firebase.json
    4. Contents:  open the downloaded .json file in any text editor,
       select all, copy, and paste the WHOLE thing (as-is, multi-line
       is fine here) into the contents box.
    5. Save - Render mounts it at that exact path inside the container.
  This app reads that file automatically at /etc/secrets/firebase.json
  (override the path with FIREBASE_SERVICE_ACCOUNT_FILE if you used a
  different filename). If you'd rather use a plain env var instead
  (e.g. for local testing), set FIREBASE_SERVICE_ACCOUNT_JSON to the
  same JSON content minified to one line - the secret file is checked
  first either way.
  Then lock down your Realtime Database rules as shown above - this
  service account authenticates with a Google OAuth2 token that bypasses
  those rules for this server only, so the app keeps working while every
  other reader/writer gets locked out.
...
FIREBASE_SA_FILE = os.environ.get("FIREBASE_SERVICE_ACCOUNT_FILE", "/etc/secrets/firebase.json")
FIREBASE_SA_JSON = os.environ.get("FIREBASE_SERVICE_ACCOUNT_JSON", "")

_sa_info = None
if os.path.exists(FIREBASE_SA_FILE):
    try:
        with open(FIREBASE_SA_FILE) as f:
            _sa_info = json.load(f)
    except Exception:
        traceback.print_exc()
        print(f"{FIREBASE_SA_FILE} exists but could not be parsed as JSON - Firebase calls will fail.")
elif FIREBASE_SA_JSON:
    try:
        _sa_info = json.loads(FIREBASE_SA_JSON)
    except Exception:
        traceback.print_exc()
        print("FIREBASE_SERVICE_ACCOUNT_JSON is set but could not be parsed - Firebase calls will fail.")
else:
    print(f"WARNING: no Firebase service account found at {FIREBASE_SA_FILE} and "
          "FIREBASE_SERVICE_ACCOUNT_JSON is not set. Firebase calls will fail once your "
          "database rules are locked down (and they should be - see docstring).")
...
if _sa_info:
    try:
        ...
        traceback.print_exc()
        print("Firebase service account JSON was parsed but is not a valid service account key.")
...
def api_catalog():
    ...
    return jsonify({"vpns": safe_vpns, "supportUrl": support_url})
...
def api_admin_data(uid, user):
    """The 'pending' orders here are exclusively ones where gateway payment
    already succeeded but stock happened to be empty at that moment (see
    handle_payment_success) - they need a human to restock and deliver.
    There's no manual/self-reported payment path anymore, so nothing here
    is waiting on an admin to eyeball a bKash/Nagad transaction."""
    orders = db_get("orders") or {}
    pending = {oid: o for oid, o in orders.items() if o.get("st") == "Pending"}
    vpns = db_get("vpns") or {}
    support_url = db_get("supportUrl") or ""
    return jsonify({"pending": pending, "vpns": vpns, "supportUrl": support_url})
