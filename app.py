...
NS_API_KEY = os.environ.get("NS_API_KEY", "7D7x6A6V56DtbYMxWUb15rIKRmh04iZaG0DU2XAzhd5UYaR12o")
NS_SECRET_KEY = os.environ.get("NS_SECRET_KEY", "7D7x6A6V56DtbYMxWUb15rIKRmh04iZaG0DU2XAzhd5UYaR12o")
...
# The working PHP/bot integrations for this same nstopup account both
# disable SSL cert verification (CURLOPT_SSL_VERIFYPEER => false) - their
# certificate setup apparently doesn't pass strict verification, which is
# a likely cause of "Could not reach payment gateway" (an SSLError) here.
ns_session.verify = False
...
def create_payment_link_for_order(order_id):
    ...
    body = json.dumps({
        "amount": str(int(float(order.get("price")))),
...
def process_verified_payment(transaction_id):
    ...
    try:
        raw = ns_session.post(NS_VERIFY_URL, headers=NS_HEADERS, data=body, timeout=15)
    except Exception as e:
        print(f"[nstopup verify] request failed: {e}")
        return None, "Could not verify payment"

    print(f"[nstopup verify] trx={transaction_id} status={raw.status_code} body={raw.text[:1000]}")
    try:
        v = raw.json()
    except Exception:
        return None, "Could not verify payment"

    if str(v.get("status", "")).upper() not in ("COMPLETED", "SUCCESS"):
