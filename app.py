...
def api_payments():
    ...
    return jsonify([{"id": pid, "name": p.get("name"), "num": p.get("num")} for pid, p in payments.items()])
...
def api_order_pay_manual():
    ...
    db_patch(f"orders/{order_id}", {
        "st": "Pending", "method": method, "trx": trx.upper(), "date": bd_now_str(),
    })
...
def api_admin_payment_add():
    ...
    if not (name and num):
        return jsonify({"error": "Missing fields"}), 400
    pid = db_push("payments", {"name": name, "num": num})
