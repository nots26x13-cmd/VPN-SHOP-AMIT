...
## If Auto Pay shows "Invalid API Response"
That message comes straight from nstopup's `/api/payment/create` endpoint
- it means nstopup rejected the request, not this app's own code.
`NS_CREATE_URL`/`NS_VERIFY_URL` now point at `gateway.nstopup.com` (the
correct API host - an earlier version of this file pointed at
`pay.nstopup.com` by mistake, which is likely why this showed up).
If it still happens, other common causes for that specific gateway
response:
- `NS_API_KEY` / `NS_SECRET_KEY` / `NS_BRAND_KEY` are still the
  placeholder defaults (not set as real values in Render's environment
  variables), or one of them is wrong/expired
- The nstopup account/merchant isn't fully activated yet on their side
- A field nstopup expects wasn't in the exact shape they want (e.g. they
  may want `amount` as a number instead of a string, or a different key
  name) - their dashboard's own API docs are the source of truth for this,
  since this isn't a documented public API

The backend now logs the exact request it sent and nstopup's exact raw
response for every Auto Pay attempt - check your Render service logs
right after a failed attempt (`[nstopup create] ...` lines) for the real
reason, and compare that against your nstopup merchant dashboard/API docs
or their support.
