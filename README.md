...
Comparing against your demo bot and a working PHP site on the same
nstopup account turned up the real bugs (all fixed now):
- **The API key defaults were wrong.** `NS_API_KEY` and `NS_SECRET_KEY`
  were still placeholder text by default - only `NS_BRAND_KEY` had a real
  key baked in. Both your working demo bot and the PHP site use the exact
  same key (`7D7x6A6V...`) for all three headers, so all three now
  default to that confirmed-working key. If Render's environment
  variables were never actually set to real values, this alone explains
  every failure so far.
- **SSL verification.** Both working references explicitly disable SSL
  cert verification when calling nstopup (`CURLOPT_SSL_VERIFYPEER =>
  false`) - nstopup's certificate setup apparently doesn't pass strict
  verification. This app now does the same (`ns_session.verify = False`),
  which is the likely cause of "Could not reach payment gateway".
- The request body was sending `"meta_data"` (underscore) instead of the
  `"metadata"` nstopup - and this app's own verify step - actually reads.
- The verify step now also accepts a `"SUCCESS"` status, not just
  `"COMPLETED"` (case-insensitive), matching the PHP reference.
- The API host is `pay.nstopup.com` (confirmed by both references).

If it still fails, the backend logs the exact request it sent and
nstopup's exact raw response for every Auto Pay attempt and verify call -
check your Render service logs (`[nstopup create] ...` / `[nstopup
verify] ...` lines) right after an attempt.
